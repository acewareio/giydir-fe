...
export type WaitlistResponse = undefined;
export function useWaitlistService() {
  const fetch = useFetch();

  return useCallback(
    (data: { email: string }, requestConfig?: RequestConfigType) => {
      return fetch(`${API_URL}/v1/waitlist/subscribe`, {
        method: "POST",
        body: JSON.stringify(data),
        ...requestConfig,
      }).then(wrapperFetchJsonResponse<WaitlistResponse>);
    },
    [fetch]
  );
}
